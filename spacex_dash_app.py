...
def get_scatter_plot(selected_site, selected_payload):
    ...
    if selected_site == 'All':
        ...
        filtered_df = filtered_df[(filtered_df['Launch Site'] == selected_site) & (filtered_df['Payload Mass (kg)'] >= selected_payload[0]) & (filtered_df['Payload Mass (kg)'] <= selected_payload[1])]
        fig = px.scatter(
            filtered_df,
            x='Payload Mass (kg)',
